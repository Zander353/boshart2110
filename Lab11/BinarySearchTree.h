...
TreeNode<T>* BinarySearchTree<T>::removeNode(TreeNode<T>* tNode)
{
   ...
   else 
   {
      //DO THIS
		...
		// overwrite the current nodes data with the IS's data(setItem(...))
		//remove the IS
		...
		tNode->setItem(findLeftMost(right));
...
T** BinarySearchTree<T>::toArray()
{
   //DO THIS
    ...
    BinaryTreeIterator<T>* bti = iterator();
	bti->setInorder();
	T** array = new T*[sze];
	int count = 0;
	while (bti->hasNext())
	{
		array[count] = bti->next();
		count++;
	}
